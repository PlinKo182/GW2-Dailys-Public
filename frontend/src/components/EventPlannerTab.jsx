...
import { Clock, Sword, Check, X } from 'lucide-react';
...
const ALL_MAP_CHESTS = [
  { id: 'verdant_brink_heros_choice_chest', name: 'Verdant Brink', region: 'Heart of Maguuma', icon: getEventIcon('Night Bosses'), eventRegion: 'Heart of Thorns', eventMap: 'Verdant Brink', eventDataKey: 'Night Bosses' },
  { id: 'auric_basin_heros_choice_chest', name: 'Auric Basin', region: 'Heart of Maguuma', icon: getEventIcon('Octovine'), eventRegion: 'Heart of Thorns', eventMap: 'Auric Basin', eventDataKey: 'Octovine' },
  { id: 'tangled_depths_heros_choice_chest', name: 'Tangled Depths', region: 'Heart of Maguuma', icon: getEventIcon('Chak Gerent'), eventRegion: 'Heart of Thorns', eventMap: 'Tangled Depths', eventDataKey: 'Chak Gerent' },
  { id: 'dragons_stand_heros_choice_chest', name: "Dragon's Stand", region: 'Heart of Maguuma', icon: getEventIcon('Start advancing on the Blighting Towers'), eventRegion: 'Heart of Thorns', eventMap: "Dragon's Stand", eventDataKey: 'Start advancing on the Blighting Towers' },
  { id: 'crystal_oasis_heros_choice_chest', name: 'Crystal Oasis', region: 'Crystal Desert', icon: getEventIcon('Rounds 1 to 3'), eventRegion: 'Path of Fire', eventMap: 'Crystal Oasis', events: [{ eventDataKey: 'Rounds 1 to 3' }, { eventDataKey: 'Pinata/Reset' }] },
  { id: 'elon_riverlands_heros_choice_chest', name: 'Elon Riverlands', region: 'Crystal Desert', icon: getEventIcon('The Path to Ascension: Augury Rock'), eventRegion: 'Path of Fire', eventMap: 'Elon Riverlands', events: [{ eventDataKey: 'The Path to Ascension: Augury Rock' }, { eventDataKey: 'Doppelganger' }] },
  { id: 'the_desolation_heros_choice_chest', name: 'The Desolation', region: 'Crystal Desert', icon: getEventIcon('Maws of Torment'), eventRegion: 'Path of Fire', eventMap: 'The Desolation', events: [{ eventDataKey: 'Maws of Torment' }, { eventDataKey: 'Junundu Rising' }] },
  { id: 'domain_of_vabbi_heros_choice_chest', name: 'Domain of Vabbi', region: 'Crystal Desert', icon: getEventIcon("Serpents' Ire"), eventRegion: 'Path of Fire', eventMap: 'Domain of Vabbi', events: [{ eventDataKey: "Serpents' Ire" }, { eventDataKey: 'Forged with Fire' }] },
  { id: 'dragons_end_heros_choice_chest', name: "Dragon's End", region: 'Cantha', icon: getEventIcon('The Battle for the Jade Sea'), eventRegion: 'End of Dragons', eventMap: "Dragon's End", eventDataKey: 'The Battle for the Jade Sea' },
  { id: 'seitung_province_heros_choice_chest', name: 'Seitung Province', region: 'Cantha', icon: getEventIcon('Aetherblade Assault'), eventRegion: 'End of Dragons', eventMap: 'Seitung Province', eventDataKey: 'Aetherblade Assault' },
  { id: 'new_kaineng_city_heros_choice_chest', name: 'New Kaineng City', region: 'Cantha', icon: getEventIcon('Kaineng Blackout'), eventRegion: 'End of Dragons', eventMap: 'New Kaineng City', eventDataKey: 'Kaineng Blackout' },
  { id: 'echovald_wilds_heros_choice_chest', name: 'Echovald Wilds', region: 'Cantha', icon: getEventIcon('Gang War'), eventRegion: 'End of Dragons', eventMap: 'The Echovald Wilds', events: [{ eventDataKey: 'Gang War' }, { eventDataKey: 'Aspenwood' }] },
  { id: 'gyala_delve_heros_choice_chest', name: 'Gyala Delve', region: 'Cantha', icon: getEventIcon('Gang War') },
  { id: 'skywatch_archipelago_heros_choice_chest', name: 'Skywatch Archipelago', region: 'Secrets of the Obscure', icon: getEventIcon("Unlocking the Wizard's Tower"), eventRegion: 'Secrets of the Obscure', eventMap: 'Skywatch Archipelago', eventDataKey: 'Unlocking the Wizard\'s Tower' },
  { id: 'amnytas_heros_choice_chest', name: 'Amnytas', region: 'Secrets of the Obscure', icon: getEventIcon('Defense of Amnytas'), eventRegion: 'Secrets of the Obscure', eventMap: 'Amnytas', eventDataKey: 'Defense of Amnytas' },
  { id: 'convergence_heros_choice_chest', name: 'Convergence', region: 'Secrets of the Obscure', icon: getEventIcon('Convergence: Outer Nayos'), eventRegion: 'Secrets of the Obscure', eventMap: 'Conv.: Outer Nayos', eventDataKey: 'Convergence: Outer Nayos' },
  { id: 'inner_nayos_heros_choice_chest', name: 'Inner Nayos', region: 'Secrets of the Obscure', icon: getEventIcon('Of Mists and Monsters') },
  { id: 'citadel_of_zakiros_heros_choice_chest', name: 'Citadel of Zakiros', region: 'Janthir Wilds', icon: getEventIcon('Of Mists and Monsters'), eventRegion: 'Janthir Wilds', eventMap: 'Janthir Syntri', eventDataKey: 'Of Mists and Monsters' },
  { id: 'wild_island_heros_choice_chest', name: 'Lowland Shore', region: 'Janthir Wilds', icon: getEventIcon('A Titanic Voyage'), eventRegion: 'Janthir Wilds', eventMap: 'Bava Nisos', eventDataKey: 'A Titanic Voyage' },
];
...
const EventPlannerTab = () => {
  ...
  // Apply filters and group by map
  const upcomingEvents = useMemo(() => {
    ...
    // Group by map name
    const groupedByMap = filtered.reduce((acc, event) => {
      const key = event.mapName;
      if (!acc[key]) {
        acc[key] = [];
      }
      acc[key].push(event);
    ...
    // Convert to array and sort by earliest event in each map
    return Object.entries(groupedByMap)
      .map(([mapName, events]) => ({
        mapName,
          ...
          <div className="relative min-h-[400px]">
            ...
            <div className="relative space-y-2 py-2">
              {upcomingEvents.length === 0 ? (
                ...
                upcomingEvents.map((mapGroup, mapIndex) => (
                  <div
                    key={`map-${mapGroup.mapName}-${mapIndex}`}
                    className="relative h-12"
                  >
                    {mapGroup.events.map((event, eventIndex) => {
